...
**Стек:** Vanilla JS (ES Modules) + Supabase (BaaS) + Tailwind CSS (CDN) + MathJax (CDN) + Chart.js (CDN)  
...
**Навигация:**
- `#bottomNav` — мобильная нижняя панель (скрыта на ≥641px через CSS)
- `#desktopNav` — десктопная шапка (скрыта на ≤640px через CSS), содержит логотип, nav-ссылки, звук/тема, аватар-пилюля, кнопка «Выйти»
- `#menuBtn` / `#navMenu` — гамбургер-меню (только мобайл)
...
- `{ applyTheme, showPage, updateUserUI, renderStreakBadge, renderXPBadge, showContinueTestBanner }` ← ui.js
- `{ registerSW }` ← pwa.js
...
2. `supabase.auth.onAuthStateChange` — обработчик событий сессии:
   ...
   - `SIGNED_IN` → setupSessionGuard → showPage('homePage')
   - `SIGNED_OUT` → teardownSessionGuard → showPage('authPage')
3. `DOMContentLoaded`:
   - Генерация плавающих математических символов
   - Синхронизация кнопок темы (`#themeToggle`, `#dhTheme`)
   - Обработка URL-параметров (`?type=recovery`)
   - Восстановление сессии через `supabase.auth.getSession()`
...
export async function getDailyLeaderboard(date)    // → результаты за конкретный день
export async function getProfileByUsername(username) // → { profile, results }
export async function getDuelHistory(userId)       // → история дуэлей с opponent
```

**Таблицы в Supabase:**
- `profiles` — username, avatar_url (linked to auth.users)
...
export function renderXPBadge()          // Обновляет #xpBadge
...
**Импортирует:**
- `{ st }` ← state.js
- `{ showPage, playSound, XP_TABLE, addXP, showXPToast, renderXPBadge, launchConfetti }` ← ui.js
...
window.exitTest()
window.finishTest()        // Сохраняет результат, начисляет XP, показывает resultsPage
window.restartTest()
...
**Импортирует:**
- `{ st }` ← state.js
- `{ showPage, renderXPBadge, renderStreakBadge }` ← ui.js
...
**Импортирует:**
- `{ st }` ← state.js
- `{ showPage, updateUserUI, renderStreakBadge, renderXPBadge }` ← ui.js
...
```js
window.showHome()
// stopTimer → showPage('homePage') → renderStreak → renderXP → updateDailyChallengeCard
...
- `{ st }` ← state.js
- `{ showPage }` ← ui.js
- `{ getUserResults, getLeaderboard }` ← supabase.js
...
// аватар, уровень, статистика (total/best/avg), достижения, последние 5 результатов
...
2. **Next.js миграция** — переход с Vanilla JS на Next.js; после — SVG-иконки вместо эмодзи
3. **Дизайн** — единый стиль, современный UI, адаптив для больших экранов
4. **Настройки в профиле** — тема и звук убрать из шапки, перенести внутрь страницы профиля
